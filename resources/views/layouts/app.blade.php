        ...
        <script>
            ...
            window.checkinApp = window.checkinApp || function () {
                return {
                    mode: 'camera',
                    manualCode: '',
                    result: { show: false, status: '', name: '', message: '' },
                    recentCheckins: [],
                    pendingCount: 0,
                    ...
                    async init() {
                        ...
                        window.addEventListener('online', function () { self.online = true; if (window.TukioCheckin) TukioCheckin.retryUnsynced(); });
                        ...
                        if (this.mode === 'camera') {
                            this.$nextTick(function () { self.startScanner(); });
                        }
                    },
                    ...
                    stopScanner() {
                        if (this.qrScanner) {
                            try { this.qrScanner.stop().catch(function () {}); } catch (e) {}
                            this.qrScanner = null;
                            this.scannerRunning = false;
                        }
                    },
                    ...
                    async handleResult(token) {
                        ...
                        this.result = { show: true, status: res.status, name: res.name, message: res.message };
                        var self = this;
                        setTimeout(function () {
                            console.log('[checkinApp] handleResult — 800ms elapsed, calling refreshList...');
                            self.refreshList();
                            if (self.mode === 'camera') self.$nextTick(function () { self.startScanner(); });
                        }, 800);
                    },
